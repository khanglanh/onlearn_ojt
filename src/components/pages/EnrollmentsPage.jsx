...
import { getAllEnrollments, unenrollFromClass, listCourses, listClasses } from '../../api/academic';
...
export default function EnrollmentsPage() {
  ...
  let filteredEnrollments = enrollments.filter(enrollment => {
    ...
    const matchesStudent = !selectedStudent || enrollment.studentId === selectedStudent;
  ...
  const paginatedEnrollments = filteredEnrollments.slice(startIdx, startIdx + ITEMS_PER_PAGE);
                          ...
                          <td>
                            <div className="student-cell">
                              <div className="student-avatar">
                                {getStudentName(enrollment.studentId).charAt(0).toUpperCase()}
                              </div>
                              <div>
                                <div className="student-name">{getStudentName(enrollment.studentId)}</div>
                                <div className="student-id">{enrollment.studentId}</div>
                              </div>
                            </div>
                          </td>
                          <td>
                            <strong>{classInfo.classCode}</strong>
                            {classInfo.room && <div className="class-room">📍 {classInfo.room}</div>}
                          </td>
                          <td>
                            <div>{classInfo.courseName}</div>
                            {classInfo.courseCode && (
                              <div className="course-code">{classInfo.courseCode}</div>
                            )}
                          </td>
